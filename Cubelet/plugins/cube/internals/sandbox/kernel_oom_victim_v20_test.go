...
func seedV20ClosedGeneration(t *testing.T, store *taskOutcomeProofStore) {
	...
	if !store.RecordVictimWindowStart("sandbox-a", 7, 10_000_000_000) { t.Fatal("start window rejected") }
	if !store.RecordVictimOutcomeObserved("sandbox-a", 7, 30_000_000_000) { t.Fatal("outcome window close rejected") }
}
...
func TestV20VictimCorrelationUsesTGIDNotVictimTID(t *testing.T) {
	...
	if !ok { t.Fatal("exact process victim event was not accepted") }
	if proof.HostPID != 4242 || proof.VictimTID != 4247 || proof.Generation != 7 { t.Fatalf("unexpected proof: %+v", proof) }
	if proof.CgroupV2Correlated || proof.CgroupV2ID != 0 { t.Fatalf("unknown cgroup must not be upgraded: %+v", proof) }
}
...
func TestV20VictimCorrelationRejectsLifetimeAndWindowMismatch(t *testing.T) {
	...
	cases := []struct{name string; mutate func(*kernelvictim.Event)}{
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T){
			store := newTaskOutcomeProofStore(); seedV20ClosedGeneration(t, store)
			e := base; tc.mutate(&e)
			if _, ok := store.FinalizeHostKernelOOMVictim("sandbox-a", e, 0, false); ok { t.Fatal("mismatched event accepted") }
		})
	}
}
...
func TestV20CgroupCorrelationIsExactAndNeverSynthesized(t *testing.T) {
	...
	if !ok || !proof.CgroupV2Correlated || proof.CgroupV2ID != 88 { t.Fatalf("exact cgroup match not proven: %+v %v", proof, ok) }

	store = newTaskOutcomeProofStore(); seedV20ClosedGeneration(t, store)
	if _, ok := store.FinalizeHostKernelOOMVictim("sandbox-a", base, 89, true); ok { t.Fatal("known cgroup mismatch accepted") }

	store = newTaskOutcomeProofStore(); seedV20ClosedGeneration(t, store)
	proof, ok = store.FinalizeHostKernelOOMVictim("sandbox-a", base, 0, false)
	if !ok || proof.CgroupV2Correlated || proof.CgroupV2ID != 0 { t.Fatalf("unavailable resolver must preserve only process proof: %+v %v", proof, ok) }
}

func TestV20CreateFenceClearsVictimAuthority(t *testing.T) {
	store := newTaskOutcomeProofStore(); seedV20ClosedGeneration(t, store)
	store.Clear("sandbox-a")
	if _, ok := store.VictimWindow("sandbox-a"); ok { t.Fatal("victim window survived Create fence") }
	if got := store.ListHostKernelOOMVictimProofs(); len(got) != 0 { t.Fatalf("victim proof survived Create: %+v", got) }
}
