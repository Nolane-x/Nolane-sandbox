...
func TestV20DecodeRawVictimEventRejectsMalformedIdentity(t *testing.T) {
	...
	for i, raw := range cases {
		if _, err := DecodeRawVictimEvent(raw); err == nil {
...
func TestV20StartTimeTicksExactOnSupportedArchitectures(t *testing.T) {
	for _, arch := range []string{"amd64", "arm64"} {
		...
		if got != 1234 {
			t.Fatalf("%s: got %d want 1234", arch, got)
		}
	}
	...
	if got != 1384 {
		t.Fatalf("got %d want 1384", got)
	}
	if _, err := StartTimeTicks(1, 0, 0, "386"); err == nil {
		t.Fatal("unsupported architecture accepted")
	}
	if _, err := StartTimeTicks(1, 0, -2, "amd64"); err == nil {
		t.Fatal("negative visible start accepted")
	}
	if _, err := StartTimeTicks(math.MaxUint64, 1, 0, "amd64"); err == nil {
...
func TestV20StoreIsBoundedAndPositiveOnly(t *testing.T) {
	...
	if _, ok := s.Find(boot, 1, 1, 0, math.MaxUint64); ok {
		t.Fatal("oldest event was not evicted")
	}
	if _, ok := s.Find(boot, 1025, 1025, 0, math.MaxUint64); !ok {
	...
	if err := s.Add(e); err != nil {
		t.Fatalf("exact duplicate must be idempotent: %v", err)
	}
	conflict := e
	conflict.VictimTID = 50_001
	if err := s.Add(conflict); err == nil {
		t.Fatal("conflicting lifetime event was merged")
	}
}
