...
func (f *Local) Acquire(ctx context.Context, req AcquireRequest) (Lease, error) {
	...
	if !ok {
		return Lease{}, realm.ErrRealmNotFound
	}
	if rec.Closed {
		return Lease{}, realm.ErrRealmClosed
	}
	...
	if op, ok := f.store.Operation(req.RealmID, req.OperationID); ok {
		if op.RequestDigest != digest {
			return Lease{}, ErrOperationCollision
		}
		switch op.Status {
		case "completed":
			wr, ok := f.store.World(req.RealmID, req.WorldID)
			if !ok || wr.Phase == realm.WorldTerminal {
				return Lease{}, ErrWorldUnavailable
			}
	...
	for _, wr := range f.store.Worlds(req.RealmID) {
		if wr.Phase != realm.WorldTerminal {
			active++
		}
	}
	if active >= rec.Spec.MaxWorlds {
		return Lease{}, ErrWorldLimit
	}
	if _, exists := f.store.World(req.RealmID, req.WorldID); exists {
		return Lease{}, realm.ErrInvalidWorld
	}
	res, err := f.capacity.ReserveWithin(ReservationRequest{OperationID: req.OperationID, RealmID: req.RealmID, Units: req.Units, ExpiresUnix: req.ExpiresUnix}, rec.Spec.ResourceBudget)
	if err != nil {
		return Lease{}, err
	}
	baselineID := ""
	if b, ok := f.baselines.Select(rec.Spec.NetworkProfile); ok {
		baselineID = b.ID
	}
	...
	if err := f.store.RecordOperation(realm.OperationRecord{RealmID: req.RealmID, OperationID: req.OperationID, RequestDigest: digest, Status: "completed", ReceiptDigest: leaseDigest(lease)}); err != nil {
		return Lease{}, ErrOutcomeUncertain
	}
	return lease, nil
}
...
func (f *Local) Spawn(ctx context.Context, req SpawnRequest) (Lease, error) {
	return f.Acquire(ctx, AcquireRequest(req))
}
...
func (f *Local) Handle(realmID realm.ID, worldID world.ID, generation uint64) (substrate.Handle, uint64, error) {
	if f == nil {
		return "", 0, ErrInvalidFabric
	}
	f.mu.Lock()
	defer f.mu.Unlock()
	wr, ok := f.store.World(realmID, worldID)
	if !ok {
		return "", 0, ErrWorldUnavailable
	}
	if wr.Phase == realm.WorldTerminal {
		return "", 0, ErrWorldTerminal
	}
	if wr.Phase != realm.WorldLeased || wr.Handle == "" {
		return "", 0, ErrWorldUnavailable
	}
	l := Lease{RealmID: wr.RealmID, WorldID: wr.WorldID, Generation: wr.LeaseGeneration, ExpiresUnix: wr.LeaseExpiresUnix, RealizationRevision: wr.RealizationRevision}
	if _, ok := f.leases.Current(realmID, worldID); !ok {
		_ = f.leases.Restore(l)
	}
	if generation != l.Generation {
		return "", 0, ErrStaleLease
	}
	if err := f.leases.Validate(l, f.now().Unix()); err != nil {
		return "", 0, err
	}
...
func (f *Local) Checkpoint(ctx context.Context, realmID realm.ID, worldID world.ID, generation uint64) (realm.CheckpointRecord, error) {
	...
	defer f.mu.Unlock()
	wr, ok := f.store.World(realmID, worldID)
	if !ok || wr.Phase != realm.WorldLeased {
		return realm.CheckpointRecord{}, ErrWorldUnavailable
	}
	l := Lease{RealmID: wr.RealmID, WorldID: wr.WorldID, Generation: wr.LeaseGeneration, ExpiresUnix: wr.LeaseExpiresUnix, RealizationRevision: wr.RealizationRevision}
	if _, ok := f.leases.Current(realmID, worldID); !ok {
		_ = f.leases.Restore(l)
	...
	if generation != l.Generation {
		return realm.CheckpointRecord{}, ErrStaleLease
	}
	if err := f.leases.Validate(l, f.now().Unix()); err != nil {
		return realm.CheckpointRecord{}, err
	}
	snap, err := f.manager.Snapshot(ctx, worldID)
	if err != nil || snap == "" {
		return realm.CheckpointRecord{}, ErrOutcomeUncertain
	}
	state, ok := f.manager.AuthorityState(worldID)
	if !ok || state.CurrentEpoch() == 0 {
		return realm.CheckpointRecord{}, ErrOutcomeUncertain
	}
	rec, ok := f.store.Realm(realmID)
	if !ok {
		return realm.CheckpointRecord{}, realm.ErrRealmNotFound
	}
	...
	if err := f.store.PutCheckpoint(cp); err != nil {
		return realm.CheckpointRecord{}, err
	}
	return cp, nil
}
...
func (f *Local) Resume(ctx context.Context, checkpoint realm.CheckpointID, expectedRealmRevision uint64) (Lease, error) {
	...
	if !ok {
		return Lease{}, realm.ErrInvalidCheckpoint
	}
	...
	if rr.Revision != expectedRealmRevision || rr.Revision != cp.RealmRevision {
		return Lease{}, ErrStaleRealmRevision
	}
	wr, ok := f.store.World(cp.RealmID, cp.WorldID)
	if !ok || wr.Phase == realm.WorldTerminal {
		return Lease{}, ErrWorldTerminal
	}
	if err := f.manager.Rollback(ctx, cp.WorldID, cp.Snapshot); err != nil {
		return Lease{}, ErrOutcomeUncertain
	}
	state, ok := f.manager.AuthorityState(cp.WorldID)
	if !ok || state.CurrentEpoch() <= cp.AuthorityEpoch {
		return Lease{}, ErrOutcomeUncertain
	}
...
func (f *Local) Release(ctx context.Context, realmID realm.ID, worldID world.ID, generation uint64) error {
	...
	if err := f.manager.Destroy(ctx, worldID); err != nil {
...
func leaseDigest(l Lease) string {
	raw, _ := json.Marshal(l)
	h := sha256.Sum256(append([]byte("nolane.fabric.lease.v1\x00"), raw...))
	return hex.EncodeToString(h[:])
}

func checkpointID(r realm.ID, w world.ID, rev uint64, epoch world.Epoch, snap substrate.Snapshot) realm.CheckpointID {
	raw, _ := json.Marshal([]any{r, w, rev, epoch, snap})
	h := sha256.Sum256(append([]byte("nolane.realm.checkpoint.v1\x00"), raw...))
	return realm.CheckpointID("checkpoint://" + hex.EncodeToString(h[:]))
}
