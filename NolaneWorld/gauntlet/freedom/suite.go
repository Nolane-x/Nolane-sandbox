...
func Standard() []gauntlet.Scenario {
	suite := []gauntlet.Scenario{
		...
		freedomScenario("freedom.realm-policy-host-only", "Realm administration remains host-only.", "Inspect the agent Runtime interface for Realm create/update/close administration.", "The semantic runtime exposes use of an existing Realm but no Realm policy mutation authority.", "realm-policy-host-only", realmPolicyHostOnly),
