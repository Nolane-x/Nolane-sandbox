...
func TestHostObserverV1NormalizesEquivalentProof(t *testing.T) {
	...
	if report := trusted.Report(); !report.Approved || report.CPU.Observation.ThrottledUsecAfter <= report.CPU.Observation.ThrottledUsecBefore || report.Memory.Observation.OOMEventsAfter <= report.Memory.Observation.OOMEventsBefore {
