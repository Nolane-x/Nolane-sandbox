...
func newRuntimeCgroupHelperExecutorForTest(cfg runtimeCgroupHelperExecutorTestConfig) *RuntimeCgroupHelperExecutor {
	...
	e.readStartTime = procStartTimeReader(e.readFile)
	e.readHelperExecutableDigest = func(int) (string, error) {
		return e.executableDigest, nil
	}
	return e
}

func (e *RuntimeCgroupHelperExecutor) valid() bool {
	if e == nil || e.seal != currentRuntimeCgroupHelperExecutorSeal || e.nonce == nil || e.launch == nil || e.readFile == nil || e.writeFile == nil || e.readStartTime == nil || e.readHelperExecutableDigest == nil || e.clock == nil {
		return false
	}
...
func measureRuntimeCgroupHelperProcessExecutable(pid int) (string, error) {
	if pid <= 0 {
		return "", ErrInvalidRuntimeCgroupHelperPlacementAuthority
	}
	return sha256File("/proc/" + strconv.Itoa(pid) + "/exe")
}
...
func waitRuntimeCgroupHelperChild(ctx context.Context, child runtimeCgroupHelperChild) (int, error) {
	if child == nil {
		return -1, ErrInvalidRuntimeCgroupHelperPlacementAuthority
	}
	type result struct {
...
func (e *RuntimeCgroupHelperExecutor) execute(ctx context.Context, binding RuntimeCgroupReadbackSnapshot) (runtimeCgroupHelperLifecycle, error) {
	...
	childExecutableDigest, err := e.readHelperExecutableDigest(pid)
	if err != nil || childExecutableDigest != e.executableDigest || !validV29SHA256(childExecutableDigest) {
		return fail(ErrInvalidRuntimeCgroupHelperPlacementAuthority)
	}
	...
	exitCode, waitErr := waitRuntimeCgroupHelperChild(ctx, child)
	waited = true
	...
	return runtimeCgroupHelperLifecycle{
		PID: pid, StartTimeTicks: startBefore, ExecutableSHA256: childExecutableDigest,
