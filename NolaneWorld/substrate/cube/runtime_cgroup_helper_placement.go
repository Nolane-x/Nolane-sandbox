...
// RuntimeCgroupHelperExecutor is Wave29 package-owned process placement
// authority. Production construction accepts no caller-selected executable,
// cgroup root, filesystem, PID, nonce, process launcher, or pressure runner.
...
func measureRuntimeCgroupHelperExecutable() (string, string, error) {
	...
	if err != nil {
		return "", "", ErrInvalidRuntimeCgroupHelperPlacementAuthority
	}
...
func launchRuntimeCgroupHelperProcess(ctx context.Context, executable, nonce string) (runtimeCgroupHelperChild, error) {
	...
	if executable == "" || !filepath.IsAbs(executable) || !canonicalV29NonceHex(nonce) {
		return nil, ErrInvalidRuntimeCgroupHelperPlacementAuthority
	}
	...
	_ = releaseR.Close()
	_ = ackW.Close()
	return &execRuntimeCgroupHelperChild{
		cmd: cmd, release: releaseW, ackFile: ackR,
...
func runtimeCgroupHelperEnvironment(nonce string) []string {
	...
	for _, entry := range os.Environ() {
		if strings.HasPrefix(entry, modePrefix) || strings.HasPrefix(entry, noncePrefix) {
			continue
		}
		out = append(out, entry)
	}
	return append(out,
		modePrefix+runtimeCgroupHelperModeParkExit,
		noncePrefix+nonce,
	)
}
...
func (c *execRuntimeCgroupHelperChild) AwaitReady(ctx context.Context, nonce string) error {
	return c.awaitRecord(ctx, "READY "+nonce+"\n", false)
}

func (c *execRuntimeCgroupHelperChild) Release(nonce string) error {
	if c == nil || c.release == nil || !canonicalV29NonceHex(nonce) {
		return ErrInvalidRuntimeCgroupHelperPlacementAuthority
	}
	_, err := io.WriteString(c.release, "GO "+nonce+"\n")
	closeErr := c.release.Close()
	c.release = nil
...
func (c *execRuntimeCgroupHelperChild) AwaitDone(ctx context.Context, nonce string) error {
	return c.awaitRecord(ctx, "DONE "+nonce+"\n", true)
}
...
func (c *execRuntimeCgroupHelperChild) awaitRecord(ctx context.Context, expected string, requireEOF bool) error {
	...
	go func() {
		line, err := c.ack.ReadSlice('\n')
		ch <- result{line: append([]byte(nil), line...), err: err}
	}()
	select {
	case <-ctx.Done():
		return ctx.Err()
	case got := <-ch:
		if got.err != nil || len(got.line) > runtimeCgroupHelperProtocolLimit || string(got.line) != expected {
			return ErrInvalidRuntimeCgroupHelperPlacementAuthority
		}
	}
	if !requireEOF {
...
func (c *execRuntimeCgroupHelperChild) Wait() (int, error) {
	if c == nil || c.cmd == nil {
		return -1, ErrInvalidRuntimeCgroupHelperPlacementAuthority
	}
	c.mu.Lock()
	if c.waited {
		c.mu.Unlock()
...
func (c *execRuntimeCgroupHelperChild) Alive() bool {
	...
	c.mu.Lock()
	defer c.mu.Unlock()
	return !c.waited && c.cmd.ProcessState == nil
}
...
func procStartTimeReader(readFile runtimeCgroupHelperReadFile) func(int) (uint64, error) {
	return func(pid int) (uint64, error) {
		if readFile == nil || pid <= 0 {
			return 0, ErrInvalidRuntimeCgroupHelperPlacementAuthority
		}
		...
		parsedPID, err := strconv.Atoi(prefix)
		...
		start, err := parseCanonicalUint(rest[19], 64)
		if err != nil || start == 0 {
			return 0, ErrInvalidRuntimeCgroupHelperPlacementAuthority
		}
		return start, nil
...
// RuntimeCgroupHelperPlacementAuthority proves exact package-owned helper
// placement/execution ordering only. It is not CPU/memory causal evidence.
...
func (a RuntimeCgroupHelperPlacementAuthority) Valid() bool {
	if a.seal != currentRuntimeCgroupHelperPlacementAuthoritySeal || !a.before.Valid() || !a.after.Valid() || !validV29AuthorityDigest(a.digest) {
		return false
	}
	before, ok1 := a.before.Snapshot()
	after, ok2 := a.after.Snapshot()
	if !ok1 || !ok2 || !sameV29ImmutableReadback(before, after) || !v29SnapshotMatchesReadbacks(a.snapshot, a.before, a.after) {
		return false
	}
	expected, err := deriveV29AuthorityDigest(a.snapshot)
	return err == nil && expected == a.digest
}

func (a RuntimeCgroupHelperPlacementAuthority) Digest() (string, bool) {
	if !a.Valid() {
		return "", false
	}
...
func (a RuntimeCgroupHelperPlacementAuthority) Snapshot() (RuntimeCgroupHelperPlacementSnapshot, bool) {
	if !a.Valid() {
		return RuntimeCgroupHelperPlacementSnapshot{}, false
	}
...
func ValidateRuntimeCgroupHelperPlacementAuthority(
	...
	if err := ctx.Err(); err != nil {
		return RuntimeCgroupHelperPlacementAuthority{}, err
	}
	if !runtimeAuthority.Valid() || !executor.valid() {
		return RuntimeCgroupHelperPlacementAuthority{}, ErrInvalidRuntimeCgroupHelperPlacementAuthority
	}

	before, err := ValidateRuntimeCgroupReadbackAuthority(
		ctx, controller, realization, runtimeAuthority, epochObserver, client, runtimeObserver, cgroupObserver,
	)
	if err != nil {
		return RuntimeCgroupHelperPlacementAuthority{}, err
	}
	beforeSnapshot, ok := before.Snapshot()
	if !ok {
		return RuntimeCgroupHelperPlacementAuthority{}, ErrInvalidRuntimeCgroupHelperPlacementAuthority
	}

	lifecycle, err := executor.execute(ctx, beforeSnapshot)
	if err != nil {
		return RuntimeCgroupHelperPlacementAuthority{}, err
	}
	if executor.afterChildExit != nil {
		executor.afterChildExit()
	}

	after, err := ValidateRuntimeCgroupReadbackAuthority(
		ctx, controller, realization, runtimeAuthority, epochObserver, client, runtimeObserver, cgroupObserver,
	)
	if err != nil {
		return RuntimeCgroupHelperPlacementAuthority{}, err
	}
	afterSnapshot, ok := after.Snapshot()
	if !ok || !sameV29ImmutableReadback(beforeSnapshot, afterSnapshot) {
		return RuntimeCgroupHelperPlacementAuthority{}, ErrInvalidRuntimeCgroupHelperPlacementAuthority
	}
	if afterSnapshot.NrThrottled < beforeSnapshot.NrThrottled || afterSnapshot.ThrottledUsec < beforeSnapshot.ThrottledUsec || afterSnapshot.OOMEvents < beforeSnapshot.OOMEvents || afterSnapshot.OOMKillEvents < beforeSnapshot.OOMKillEvents {
		return RuntimeCgroupHelperPlacementAuthority{}, ErrInvalidRuntimeCgroupHelperPlacementAuthority
	}
	beforeDigest, ok1 := before.ReadbackDigest()
	afterDigest, ok2 := after.ReadbackDigest()
	if !ok1 || !ok2 {
		return RuntimeCgroupHelperPlacementAuthority{}, ErrInvalidRuntimeCgroupHelperPlacementAuthority
	}

	snapshot := RuntimeCgroupHelperPlacementSnapshot{
		RuntimeDigest:          beforeSnapshot.RuntimeDigest,
		BeforeReadbackDigest:   beforeDigest,
		AfterReadbackDigest:    afterDigest,
		SandboxID:              beforeSnapshot.SandboxID,
		Generation:             beforeSnapshot.Generation,
		RuntimeHostPID:         beforeSnapshot.HostPID,
		RuntimeStartTimeTicks:  beforeSnapshot.StartTimeTicks,
		RuntimeBootID:          beforeSnapshot.BootID,
		CGroupPath:             beforeSnapshot.CGroupPath,
		CPUQuotaMicros:         beforeSnapshot.CPUQuotaMicros,
		CPUPeriodMicros:        beforeSnapshot.CPUPeriodMicros,
		MemoryLimitBytes:       beforeSnapshot.MemoryLimitBytes,
		HelperPID:              lifecycle.PID,
		HelperStartTimeTicks:   lifecycle.StartTimeTicks,
		HelperExecutableSHA256: lifecycle.ExecutableSHA256,
		HelperNonceSHA256:      lifecycle.NonceSHA256,
		ReadyAt:                lifecycle.ReadyAt,
		PlacedAt:               lifecycle.PlacedAt,
		ReleasedAt:             lifecycle.ReleasedAt,
		ExitedAt:               lifecycle.ExitedAt,
		ExitCode:               lifecycle.ExitCode,
	}
	digest, err := deriveV29AuthorityDigest(snapshot)
	if err != nil {
		return RuntimeCgroupHelperPlacementAuthority{}, err
	}
	authority := RuntimeCgroupHelperPlacementAuthority{
		before: before, after: after, snapshot: snapshot, digest: digest,
		seal: currentRuntimeCgroupHelperPlacementAuthoritySeal,
	}
	if !authority.Valid() {
		return RuntimeCgroupHelperPlacementAuthority{}, ErrInvalidRuntimeCgroupHelperPlacementAuthority
	}
...
func (e *RuntimeCgroupHelperExecutor) execute(ctx context.Context, binding RuntimeCgroupReadbackSnapshot) (runtimeCgroupHelperLifecycle, error) {
	if err := ctx.Err(); err != nil {
		return runtimeCgroupHelperLifecycle{}, err
	}
	if !e.valid() || binding.CGroupPath == "" || binding.SandboxID == "" {
		return runtimeCgroupHelperLifecycle{}, ErrInvalidRuntimeCgroupHelperPlacementAuthority
	}
	...
	fail := func(err error) (runtimeCgroupHelperLifecycle, error) {
		cleanup()
		if ctxErr := ctx.Err(); ctxErr != nil {
			return runtimeCgroupHelperLifecycle{}, ctxErr
		}
		if err == nil {
			err = ErrInvalidRuntimeCgroupHelperPlacementAuthority
		}
		return runtimeCgroupHelperLifecycle{}, err
	}
	...
	if pid <= 0 || uint64(pid) > uint64(^uint32(0)) || !child.Alive() {
		return fail(ErrInvalidRuntimeCgroupHelperPlacementAuthority)
	}
	startBefore, err := e.readStartTime(pid)
	if err != nil || startBefore == 0 {
		return fail(ErrInvalidRuntimeCgroupHelperPlacementAuthority)
	}
	target, err := runtimeCgroupHelperTargetPath(e.root, binding.CGroupPath)
	if err != nil {
		return fail(err)
	}
	...
	startAfter, err := e.readStartTime(pid)
	if err != nil || startAfter != startBefore || !child.Alive() {
		return fail(ErrInvalidRuntimeCgroupHelperPlacementAuthority)
	}
	placedAt := e.clock().UTC()
	if err := child.Release(nonce); err != nil {
		return fail(err)
	}
	releasedAt := e.clock().UTC()
	...
	return runtimeCgroupHelperLifecycle{
		PID: pid, StartTimeTicks: startBefore, ExecutableSHA256: e.executableDigest,
		NonceSHA256: nonceDigest, ReadyAt: readyAt, PlacedAt: placedAt,
		ReleasedAt: releasedAt, ExitedAt: exitedAt, ExitCode: exitCode,
	}, nil
}

func runtimeCgroupHelperTargetPath(root, cgroupPath string) (string, error) {
	if root == "" || cgroupPath == "" || !filepath.IsAbs(root) || !path.IsAbs(cgroupPath) || cgroupPath == "/" || path.Clean(cgroupPath) != cgroupPath {
		return "", ErrInvalidRuntimeCgroupHelperPlacementAuthority
	}
	cleanRoot := filepath.Clean(root)
	if cleanRoot != root || cleanRoot == string(filepath.Separator) {
		return "", ErrInvalidRuntimeCgroupHelperPlacementAuthority
	}
...
func sameV29ImmutableReadback(a, b RuntimeCgroupReadbackSnapshot) bool {
	return a.RuntimeDigest == b.RuntimeDigest && a.SandboxID == b.SandboxID && a.Generation == b.Generation &&
		a.HostPID == b.HostPID && a.StartTimeTicks == b.StartTimeTicks && a.BootID == b.BootID &&
		a.CGroupPath == b.CGroupPath && a.CPUQuotaMicros == b.CPUQuotaMicros &&
		a.CPUPeriodMicros == b.CPUPeriodMicros && a.MemoryLimitBytes == b.MemoryLimitBytes
}
...
func v29SnapshotMatchesReadbacks(snapshot RuntimeCgroupHelperPlacementSnapshot, before, after RuntimeCgroupReadbackAuthority) bool {
	beforeSnapshot, ok1 := before.Snapshot()
	afterSnapshot, ok2 := after.Snapshot()
	beforeDigest, ok3 := before.ReadbackDigest()
	afterDigest, ok4 := after.ReadbackDigest()
	if !ok1 || !ok2 || !ok3 || !ok4 || !sameV29ImmutableReadback(beforeSnapshot, afterSnapshot) {
		return false
	}
	return snapshot.RuntimeDigest == beforeSnapshot.RuntimeDigest &&
		snapshot.BeforeReadbackDigest == beforeDigest && snapshot.AfterReadbackDigest == afterDigest &&
		snapshot.SandboxID == beforeSnapshot.SandboxID && snapshot.Generation == beforeSnapshot.Generation &&
		snapshot.RuntimeHostPID == beforeSnapshot.HostPID && snapshot.RuntimeStartTimeTicks == beforeSnapshot.StartTimeTicks &&
		snapshot.RuntimeBootID == beforeSnapshot.BootID && snapshot.CGroupPath == beforeSnapshot.CGroupPath &&
		snapshot.CPUQuotaMicros == beforeSnapshot.CPUQuotaMicros && snapshot.CPUPeriodMicros == beforeSnapshot.CPUPeriodMicros &&
		snapshot.MemoryLimitBytes == beforeSnapshot.MemoryLimitBytes && validV29PlacementSnapshot(snapshot)
}

func validV29PlacementSnapshot(s RuntimeCgroupHelperPlacementSnapshot) bool {
	if !strings.HasPrefix(s.RuntimeDigest, "runtime-realization-v27:") || !validRuntimeCgroupReadbackDigest(s.BeforeReadbackDigest) || !validRuntimeCgroupReadbackDigest(s.AfterReadbackDigest) {
		return false
	}
	...
	if s.CGroupPath == "" || path.Clean(s.CGroupPath) != s.CGroupPath || s.CGroupPath == "/" || s.CPUQuotaMicros <= 0 || s.CPUPeriodMicros == 0 || s.MemoryLimitBytes == 0 {
		return false
	}
	...
	if s.ReadyAt.IsZero() || s.PlacedAt.IsZero() || s.ReleasedAt.IsZero() || s.ExitedAt.IsZero() {
		return false
	}
...
func deriveV29AuthorityDigest(snapshot RuntimeCgroupHelperPlacementSnapshot) (string, error) {
	if !validV29PlacementSnapshot(snapshot) {
		return "", ErrInvalidRuntimeCgroupHelperPlacementAuthority
	}
	...
	h := sha256.New()
	_, _ = h.Write([]byte(runtimeCgroupHelperDigestDomain))
	_, _ = h.Write(raw)
	return runtimeCgroupHelperDigestPrefix + hex.EncodeToString(h.Sum(nil)), nil
}

func validV29AuthorityDigest(raw string) bool {
	if !strings.HasPrefix(raw, runtimeCgroupHelperDigestPrefix) {
		return false
	}
	return validV29SHA256(strings.TrimPrefix(raw, runtimeCgroupHelperDigestPrefix))
}

func validV29SHA256(raw string) bool {
