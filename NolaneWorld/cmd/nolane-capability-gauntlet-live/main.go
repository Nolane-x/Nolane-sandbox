...
func run(args []string, getenv func(string) string, stdout, stderr io.Writer) int {
	fs := flag.NewFlagSet("nolane-capability-gauntlet-live", flag.ContinueOnError)
	fs.SetOutput(stderr)
	...
	body, marshalErr := capabilityproof.MarshalReport(report, credentialEncodings(apiKey)...)
	if marshalErr != nil {
		fmt.Fprintln(stderr, "capability live evidence verification failed")
		return 1
	}

	if *outPath != "" {
		if err := writeAtomic(*outPath, body); err != nil {
			fmt.Fprintln(stderr, "write capability live evidence failed")
			return 1
		}
	} else {
	...
	switch {
	case errors.Is(runErr, capabilityproof.ErrUnavailable):
		return 2
	case errors.Is(runErr, capabilityproof.ErrFailed):
		return 1
	case runErr != nil:
		return 1
	default:
...
func writeAtomic(path string, data []byte) error {
	...
	file, err := os.CreateTemp(dir, ".nolane-capability-live-*.tmp")
	if err != nil {
		return err
	}
	tmp := file.Name()
