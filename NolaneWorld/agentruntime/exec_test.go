...
func (f *fakeFabric) Acquire(context.Context, fabric.AcquireRequest) (fabric.Lease, error) {
	return f.lease, nil
}
func (f *fakeFabric) Spawn(context.Context, fabric.SpawnRequest) (fabric.Lease, error) {
	return f.lease, nil
}
func (f *fakeFabric) Release(context.Context, realm.ID, world.ID, uint64) error { return nil }
func (f *fakeFabric) Handle(_ realm.ID, _ world.ID, generation uint64) (substrate.Handle, uint64, error) {
	f.handleCalls++
	if generation != f.lease.Generation {
		return "", 0, fabric.ErrStaleLease
	}
	return f.handle, f.revision, nil
}
func (f *fakeFabric) Checkpoint(context.Context, realm.ID, world.ID, uint64) (realm.CheckpointRecord, error) {
	return realm.CheckpointRecord{}, errors.New("not used")
}
func (f *fakeFabric) Resume(context.Context, realm.CheckpointID, uint64) (fabric.Lease, error) {
	return fabric.Lease{}, errors.New("not used")
}
...
func (g *fakeGuest) Exec(context.Context, substrate.Handle, substrate.ProcessRequest) (substrate.ProcessObservation, error) {
	g.calls++
	if g.err != nil {
		return substrate.ProcessObservation{}, g.err
	}
	return substrate.ProcessObservation{ExitCode: 0, Stdout: []byte("ok"), ObservationDigest: "abcd"}, nil
}
...
func TestExecExactReplayAndChangedPayloadCollision(t *testing.T) {
	...
	first, err := svc.Exec(context.Background(), req)
	if err != nil {
		t.Fatal(err)
	}
	second, err := svc.Exec(context.Background(), req)
	if err != nil {
		t.Fatal(err)
	}
	if first != second {
		t.Fatalf("replay changed receipt: %+v %+v", first, second)
	}
	if guest.calls != 1 || ff.handleCalls != 1 {
		t.Fatalf("calls guest=%d handle=%d", guest.calls, ff.handleCalls)
	}
	changed := req
	changed.Command = "printf changed"
	if _, err := svc.Exec(context.Background(), changed); !errors.Is(err, ErrOperationCollision) {
...
func TestExecFailureIsUncertainAndNotReplayed(t *testing.T) {
	...
	if _, err := svc.Exec(context.Background(), req); !errors.Is(err, ErrExecUncertain) {
	...
	if guest.calls != 1 {
		t.Fatalf("guest calls=%d want 1", guest.calls)
	}
}
...
func TestStaleRealmSessionDeniedBeforeGuest(t *testing.T) {
	...
	if _, err := svc.Exec(context.Background(), req); !errors.Is(err, ErrStaleSession) {
		t.Fatalf("err=%v", err)
	}
	if guest.calls != 0 {
		t.Fatalf("guest calls=%d", guest.calls)
	}
}
