...
func TestRealmStoresRejectMalformedServiceAndOperationRows(t *testing.T) {
	...
	for _, factory := range factories {
		t.Run(factory.name, func(t *testing.T) {
			...
			badRevision := ServiceRecord{
				ID: serviceID(spec.ID, "api"), RealmID: spec.ID, WorldID: wr.WorldID,
				RealizationRevision: wr.RealizationRevision + 1, Protocol: ServiceHTTP, Port: 8080, Generation: 1, Ready: true,
			}
			if err := store.PutService(badRevision); !errors.Is(err, ErrInvalidService) {
				t.Fatalf("unowned realization revision accepted: %v", err)
			}
...
func TestRealmStoresRejectOperationStateRegression(t *testing.T) {
	factories := []struct {
		name string
		open func(*testing.T) Store
	}{
	...
	for _, factory := range factories {
		t.Run(factory.name, func(t *testing.T) {
			...
			spec := validSpec()
			if _, err := store.CreateRealm(spec); err != nil { t.Fatal(err) }
			completed := OperationRecord{RealmID: spec.ID, OperationID: "op-a", RequestDigest: "digest-a", Status: "completed", ReceiptDigest: "receipt-a"}
			if err := store.RecordOperation(completed); err != nil { t.Fatal(err) }
			regressed := completed
			regressed.Status = "uncertain"
			if err := store.RecordOperation(regressed); !errors.Is(err, ErrInvalidOperation) {
				t.Fatalf("completed operation regressed: %v", err)
			}
		})
	}
}
